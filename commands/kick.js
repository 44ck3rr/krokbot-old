module.exports = {
    run: async (message, args) => {
        ...
        const member = message.mention.members.first()
        if (!member) return message.channel.send('Veuillez mentionner le membre à exclure.')
        if (member.id === message.guild.ownerID) return message.channel.send('Vous ne pouvez pas exclure le fondateur du serveur.')
